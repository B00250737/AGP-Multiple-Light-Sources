...
void main(void) {
	...
	vec4 litColour = vec4(tmp_Color.rgb *attenuation, 1);
	vec4 amb=min(ambientI,vec4(1.0f));
		
	//out_Color=min(litColour+amb*texture(textureUnit0, ex_TexCoord),vec4(1.0f));//Here attenuation does not affect ambient
