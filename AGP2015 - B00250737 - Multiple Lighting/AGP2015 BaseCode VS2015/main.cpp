...
glm::vec4 lightPos(-5.0f, 2.0f, 2.0f, 1.0f); //light0 position
rt3d::lightStruct light1 = {
	...
	0.09f, //Attenuation - Linear
	0.032f //Attenuation - Quadratic
};glm::vec4 lightPos1(5.0f, 2.0f, 2.0f, 1.0f); //light1 position
rt3d::lightStruct light2 = {
	...
	0.09f, //Attenuation - Linear
	0.032f //Attenuation - Quadratic
};glm::vec4 lightPos2(-8.0f, 2.0f, -7.0f, 1.0f); //light2 position
...
//float attLinear = 0.14f;
//float attQuadratic = 0.007f;
...
void init(void) {
	...
	rt3d::setLight(shaderProgram, light0, 0);
	rt3d::setLight(shaderProgram, light1, 1);
	rt3d::setLight(shaderProgram, light2, 2);
...
*/
	...
	//rt3d::setLight(reflectionMapProgram, light0, 0);
...
void update(void) {
	...
	{
		if (lightChoice == 0) { lightPos[2] -= 0.1; }
		if (lightChoice == 1) { lightPos1[2] -= 0.1; }
		if (lightChoice == 2) { lightPos2[2] -= 0.1; }
	}
	if (keys[SDL_SCANCODE_J])
	{
		if (lightChoice == 0) { lightPos[0] -= 0.1; }
		if (lightChoice == 1) { lightPos1[0] -= 0.1; }
		if (lightChoice == 2) { lightPos2[0] -= 0.1; }
	}
	...
	{
		if (lightChoice == 0) { lightPos[1] += 0.1; }
		if (lightChoice == 1) { lightPos1[1] += 0.1; }
		if (lightChoice == 2) { lightPos2[1] += 0.1; }
	}
...
void draw(SDL_Window * window) {
	...
	// skybox as single cube using cube map
		...
		glm::vec4 tmp = mvStack.top()*lightPos; // Do for multiple lights
		light0.position[0] = tmp.x;
		light0.position[1] = tmp.y;
		light0.position[2] = tmp.z;
		rt3d::setLightPos(shaderProgram, glm::value_ptr(tmp), 0);

		glm::vec4 tmp1 = mvStack.top()*lightPos1;
		...
		glm::vec4 tmp2 = mvStack.top()*lightPos2;
		light2.position[0] = tmp2.x;
		light2.position[1] = tmp2.y;
		light2.position[2] = tmp2.z;
		...
		glBindTexture(GL_TEXTURE_2D, textures[0]);
		mvStack.push(mvStack.top());
		mvStack.top() = glm::translate(mvStack.top(), glm::vec3(lightPos1[0], lightPos1[1], lightPos1[2]));
		mvStack.top() = glm::scale(mvStack.top(), glm::vec3(0.25f, 0.25f, 0.25f));
		rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
		rt3d::setMaterial(shaderProgram, material0);
		rt3d::drawIndexedMesh(meshObjects[0], meshIndexCount, GL_TRIANGLES);
		...
		glBindTexture(GL_TEXTURE_2D, textures[0]);
		mvStack.push(mvStack.top());
		mvStack.top() = glm::translate(mvStack.top(), glm::vec3(lightPos2[0], lightPos2[1], lightPos2[2]));
		mvStack.top() = glm::scale(mvStack.top(), glm::vec3(0.25f, 0.25f, 0.25f));
		rt3d::setUniformMatrix4fv(shaderProgram, "modelview", glm::value_ptr(mvStack.top()));
		rt3d::setMaterial(shaderProgram, material0);
		rt3d::drawIndexedMesh(meshObjects[0], meshIndexCount, GL_TRIANGLES);
		...
		{
			mvStack.push(mvStack.top());
			mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-10.0f + b * 5, 1.0f, -16.0f + b * 2));
